function nearestSPDinv(A)
    ...
    while p == 1
        ...
        if  p == 1
            # Ahat is not pos def. It must have been just a hair off,
            ...
            Ahat = Ahat + (-mineig*k.^2 .+ eps(maxeig)*Matrix(I,size(A)[1],size(A)[1]))
        end
    
    end
    
    invAhat = inv(Ahat)
    #out = 0.5*(invAhat + invAhat')
    return invAhat
