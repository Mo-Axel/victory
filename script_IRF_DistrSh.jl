...
CSV.write(savedir * sName * "_IRF_PhatDens_DistrSh_pmean.csv", DataFrame(PhatDens_IRF))
CSV.write(savedir * sName * "_IRF_YY_DistrSh_pmean.csv", DataFrame(YY_IRF))
...
for pp = 1:n_subseq
    ...
    CSV.write(savedir * sName * "_IRF_PhatDens_DistrSh" * "_" * string(pp) * ".csv", DataFrame(PhatDens_IRF))
    CSV.write(savedir * sName * "_IRF_YY_DistrSh" * "_" * string(pp) * ".csv", DataFrame(YY_IRF))
