...
% load Agg IRFs
YY_IRF = csvread( [irfDir, sName, '_IRF_YY_AggSh1_pmean.csv'], 1, 0); % fVAR IRF
[H, n_all] = size(YY_IRF);
H = H-1;
...
%GDP_IRF_10th = quantile(GDP_IRF,0.1,2);
%GDP_IRF_50th = quantile(GDP_IRF,0.5,2);
%GDP_IRF_90th = quantile(GDP_IRF,0.9,2);
...
%PhatDens_IRF = csvread( [irfDir, sName, '_IRF_PhatDens_AggSh1_pmean.csv'], 1, 0); 
...
for ii = 1:length(vec_hh)
    ...
    figure(3+ii);clf;
    set(figure(3+ii),'PaperType','usletter','PaperOrientation','Landscape','PaperPosition',[0.1 0.1 11 8.5]);
    ...
    sFigName = [sName, '_IRF_AggSh1_DensDiff_h', num2str(hh), '.pdf'];
    saveas(figure(3+ii), [figDir '\' sFigName] );
    close all
...
%BelowCutoff_IRF = csvread( [irfDir, sName, '_IRF_BelowCutoff_AggSh1_pmean.csv'], 1, 0); 
BelowCutoff_IRF_uncertainty = csvread( [irfDir, sName, '_IRF_BelowCutoff_AggSh1_uncertainty.csv'], 1, 0); 
...
plot(-1:hh,100*(baseline+BelowCutoff_IRF_q10(1:hh+2,1)),'Color','b','LineStyle','--','LineWidth',4)  
hold on
plot(-1:hh,100*(baseline+BelowCutoff_IRF_q50(1:hh+2,1)),'Color','b','LineStyle','-','LineWidth',4) 
hold on
plot(-1:hh,100*(baseline+BelowCutoff_IRF_q90(1:hh+2,1)),'Color','b','LineStyle','--','LineWidth',4) 
hold on
plot(-1:hh, 100*baseline*ones(size(-1:hh)), 'Color', 'k', 'LineStyle', '-', 'LineWidth',2)
set(gca,'FontSize',50)
xlim([-1 hh])
ylim(100*[0.43 0.445])

sFigName = [sName, '_IRF_AggSh1_BelowCutoff.pdf'];
...
%Gini_IRF = csvread( [irfDir, sName, '_IRF_Gini_AggSh1_pmean.csv'], 1, 0); 
Gini_IRF_uncertainty = csvread( [irfDir, sName, '_IRF_Gini_AggSh1_uncertainty.csv'], 1, 0); 
...
sFigName = [sName, '_IRF_AggSh1_Gini.pdf'];
...
%Pctl_IRF = csvread( [irfDir, sName, '_IRF_Pctl_AggSh1_pmean.csv'], 1, 0); 
Pctl_IRF_uncertainty = zeros(H+1, length(vec_Pctl), n_drawsread);

for pp = 1:n_drawsread
    Pctl_IRF_uncertainty(:,:,pp)=csvread( [irfDir, sName, '_IRF_Pctl_AggSh1_', num2str(pp), '.csv'], 1, 0);
end
